...
private:
	...
	UINT32                        _baseRenderLoopMessage;
	_wli::base_render_loop        _baseRenderLoop;
...
public:
	...
	_wli::styler<render_window_main> style{this};

protected:
	render_window_main() :
		wnd(_hWnd), base_msg_pubm(_baseMsg), base_thread_pubm(_baseThread), base_text_pubm(_hWnd), _baseRenderLoopMessage(RegisterWindowMessageW(L"Render Loop")),  _baseRenderLoop(_baseRenderLoopMessage)
	{
	...
	auto renderloop_message() const noexcept { return _baseRenderLoopMessage; }

public:
	render_window_main(render_window_main&&) = default;
	render_window_main& operator=(render_window_main&&) = default; // movable only
	...
	int winmain_run(HINSTANCE hInst, int cmdShow) {
		...
		return this->_baseRenderLoop.run_loop(this->hwnd(), this->setup.accelTable); // can be used as program return value
	}
